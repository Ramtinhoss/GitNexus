...
```text
严格按照 https://raw.githubusercontent.com/nantas/GitNexus/refs/heads/nantas-dev/INSTALL-GUIDE.md 在当前仓库完成 GitNexus 首次安装、project 级 setup、索引构建和检索验收；先安装本次发布版本 @veewo/gitnexus@1.4.8，再执行 setup 写入 config.json，不要简化步骤，也不要绕过文档里的 scope 和 C# define 要求。
```
