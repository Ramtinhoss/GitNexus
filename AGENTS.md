...
> Runtime retrieval sequence is `discovery -> seed narrowing -> closure verification`; treat `resource_heuristic` as clue-tier (`clue`) evidence, and remember strong graph hops can coexist with failed closure while verifier-core remains `failed`.
