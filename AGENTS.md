...
> `query/context` default to the slim agent-safe contract. When tooling, benchmarks, or docs still need the legacy heavy payload (`processes`, `process_symbols`, `definitions`, `resourceBindings`, `serializedFields`, `next_hops`), pass `response_profile=full` explicitly.
