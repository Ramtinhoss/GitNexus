...
> `query/context` slim guidance is narrowing-first: in strict-anchor mode read `facts -> closure -> clues`, then inspect `decision.recommended_follow_up`, `missing_proof_targets`, and `suggested_context_targets` before upgrading to `response_profile=full`.
