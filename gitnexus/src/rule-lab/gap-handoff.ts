...
function isAcceptedRow(row: GapCandidateRow): boolean {
  const status = String(row.status || row.lifecycle_stage || '').toLowerCase();
...
export async function loadGapHandoff(input: {
  ...
  assertNoPlaceholders(input.runId, input.sliceId, acceptedRows);
  ...
  const handoff: RuleLabSourceGapHandoff = {
    ...
    promotion_backlog_count: toNumber(classificationBuckets.promotion_backlog?.count),
    reject_buckets: readRejectBuckets(classificationBuckets),
