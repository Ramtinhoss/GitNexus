...
- `verification_rules`
  - used during `query/context --runtime-chain-verify on-demand`
- `retrieval_rules`
...
### 2.3 `trigger_tokens` Mainly Drives Rule Recall at Retrieval Time

Current verifier architecture is:
...
- `trigger_tokens` is not "edge filtering" logic
- it is "rule recall and ranking" logic before edge verification runs

### 2.4 Fallback if `trigger_tokens` Is Missing
...
- current verifier still requires some text-oriented entry signal
- removing `trigger_tokens` without redesign just shifts dependency onto `trigger_family`
