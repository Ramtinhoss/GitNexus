...
Task 7 | completed | Added semantic gate fields to repeatability runner and unit test (`semantic_contract.coreAdjustedDelta`, `downgradeOnlyWhenStrictFallback`); unit test passes; dist runner failed with missing `dist/core/search/bm25-index.js`, artifact regenerated via source runner (`tsx`) and markdown report added; pending commit for this task.
