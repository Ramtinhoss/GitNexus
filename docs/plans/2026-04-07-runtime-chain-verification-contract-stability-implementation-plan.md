...
Task 5 | completed | Added `adjustRuntimeClaimForPolicy()` and metadata fields (`verification_core_status`, `verification_core_evidence_level`, `policy_adjusted`, `policy_adjust_reason`); wired helper in both query/context paths; `npm --prefix gitnexus exec vitest run gitnexus/test/unit/runtime-claim.policy.test.ts` passes; committed as `b201bfb`.
Task 6 | completed | Extended integration checks for `runtime_claim` core-vs-adjusted metadata in query/context and strict-fallback determinism assertions; `npm --prefix gitnexus exec vitest run gitnexus/test/integration/local-backend-calltool.test.ts gitnexus/src/mcp/local/runtime-chain-verify.test.ts` passes; committed as `8442cab`.
Task 7 | completed | Added semantic gate fields to repeatability runner and unit test (`semantic_contract.coreAdjustedDelta`, `downgradeOnlyWhenStrictFallback`); unit test passes; dist runner failed with missing `dist/core/search/bm25-index.js`, artifact regenerated via source runner (`tsx`) and markdown report added; committed as `ed7a36a`.
Task 8 | completed | Updated `_shared` + all `gitnexus-*.md` skill templates with `verifier-core` vs `policy-adjusted` closure guard and strict-fallback parity rerun rule; gate `rg -q \"fallbackToCompact|policy-adjusted|rerun.*parity\"` over source templates returns zero missing entries; committed as `dbc5632`.
Task 9 | completed | Ran setup sync flow: `npm --prefix gitnexus run build`, `node gitnexus/dist/cli/index.js setup --scope project --agent codex`; source-vs-installed skill diffs are clean (`diff -u ...` no output); `setup.test`/`ai-context.test` verified via `node --test gitnexus/dist/cli/setup.test.js gitnexus/dist/cli/ai-context.test.js`; pending commit for this task.
Task 10 | completed | Full regression checks passed: build, targeted vitest runtime-chain suites, setup/ai-context node:test suites, and contract grep gate; created `docs/reports/2026-04-07-runtime-chain-contract-stability-validation.md` and `...summary.md`; pending commit for this task.
