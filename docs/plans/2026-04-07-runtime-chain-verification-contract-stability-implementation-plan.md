...
Task 8 | completed | Updated `_shared` + all `gitnexus-*.md` skill templates with `verifier-core` vs `policy-adjusted` closure guard and strict-fallback parity rerun rule; gate `rg -q \"fallbackToCompact|policy-adjusted|rerun.*parity\"` over source templates returns zero missing entries; pending commit for this task.
