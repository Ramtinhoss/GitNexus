...
> If "Index is stale" → first ask user whether to rebuild index with `npx -y gitnexus analyze` (default reuses previous analyze scope/options; add `--no-reuse-options` to reset). If user chooses not to rebuild, explicitly state refactoring safety results may not reflect current codebase.
