...
> If step 2 says "Index is stale" → first ask user whether to rebuild index with `npx -y gitnexus analyze` (default reuses previous analyze scope/options; add `--no-reuse-options` to reset). If user chooses not to rebuild, explicitly state retrieval may not reflect current codebase.
