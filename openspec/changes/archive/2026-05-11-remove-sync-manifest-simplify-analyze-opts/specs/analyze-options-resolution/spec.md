...
`scopeRules` is resolved via the `--scope` CLI flag (comma-separated path-prefix rules). When `--scope` is provided, it overrides any stored `scopeRules`. When omitted, stored `scopeRules` are reused (after validation).
